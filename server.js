...
app.post("/api/auth/tourist", (req, res) => {
  ...
  // Find tourist by ID or Email (case-insensitive)
  const tourist = db.tourists.find(t => 
    t.id === touristId || 
    (t.email && t.email.trim().toLowerCase() === touristId.trim().toLowerCase())
  );
  if (!tourist) {
  ...
  console.log(`[AUTH] Tourist login: ${tourist.fullName} (${tourist.id})`);
...
app.post("/api/auth/admin", (req, res) => {
  ...
  const admins = db.admins || [{ username: "admin", passwordHash: "admin@123", fullName: "System Administrator", role: "Super Admin" }];
