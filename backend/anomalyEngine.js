...
async function triggerAlert(tourist, riskLevel, reason) {
  try {
    ...
    const newAlert = {
      id: alertId, tourist_id: tourist.id, tourist_name: touristName,
...
async function checkAnomalies() {
  try {
    ...
    for (const t of inactiveRes.rows || []) {
      ...
      if (!hasActiveAlert) {
        await triggerAlert({ ...t, tourist_name: t.tourist_name || "Unknown Tourist" }, "Medium Risk", reason);
      }
    }
    ...
    for (const t of stuckRes.rows || []) {
      ...
      const hasActiveAlert = activeAlerts.some(a => a.tourist_id === t.id && a.reason === reason);
      if (!hasActiveAlert) {
        await triggerAlert({ ...t, tourist_name: t.tourist_name || "Unknown Tourist" }, "Low Risk", reason);
      }
    }
    ...
    for (const t of sosSpamRes.rows || []) {
      ...
      if (!hasActiveAlert) {
        await triggerAlert({ id: t.tourist_id, tourist_name: "Unknown Tourist", x: 0, y: 0 }, "Critical", reason);
      }
    }
  } catch (err) {
